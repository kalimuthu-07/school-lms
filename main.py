...
def submit_assignment(assignment_id:int, student_id:int, file: UploadFile = File(...)):
    ...
    cursor.execute(query,(assignment_id,student_id,file_location))
...
def create_staff(username:str, password:str, school_id:int):
...
async def upload_gallery(
    ...
    try:
        ...
        file_path = f"uploads/gallery/{file.filename}"
...
def get_lectures(class_name: str, school_id: int):
    ...
    # ✅ video path fix
    for row in data:
        if not row["video"].startswith("uploads/"):
            row["video"] = "uploads/" + row["video"]
...
async def upload_book(
    ...
    cursor.execute("""
        ...
        VALUES (%s,%s,%s,%s,%s,%s)
    """, (school_id, class_name, subject, title, author, file_path))
